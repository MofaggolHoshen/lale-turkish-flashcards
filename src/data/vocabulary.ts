import {
  MessageCircle,
  UserRound,
  HelpCircle,
  Hash,
  CalendarDays,
  Clock,
  Users,
  Palette,
  Utensils,
  Carrot,
  Home as HomeIcon,
  Trees,
  PawPrint,
  Zap,
  SlidersHorizontal,
  Sparkles,
  MapPin,
} from "lucide-react";
import type { Category, Dictionary } from "../types";

export const CATEGORY_ORDER: Category[] = [
  "greetings",
  "daily_expressions",
  "pronouns",
  "questions",
  "numbers",
  "days",
  "time",
  "family",
  "body",
  "colors",
  "food",
  "vegetables",
  "household",
  "places",
  "nature",
  "animals",
  "verbs",
  "adjectives",
];

export const CATEGORY_META = {
  greetings: { label: "Greetings & Basics", icon: MessageCircle },
  daily_expressions: { label: "Daily Expressions", icon: Sparkles },
  pronouns: { label: "Pronouns", icon: UserRound },
  questions: { label: "Question Words", icon: HelpCircle },
  numbers: { label: "Numbers", icon: Hash },
  days: { label: "Days of the Week", icon: CalendarDays },
  time: { label: "Time", icon: Clock },
  family: { label: "Family", icon: Users },
  body: { label: "Human Body", icon: Users },
  colors: { label: "Colors", icon: Palette },
  food: { label: "Food & Drink", icon: Utensils },
  vegetables: { label: "Vegetables", icon: Carrot },
  household: { label: "Household", icon: HomeIcon },
  places: { label: "Places & City Life", icon: MapPin },
  nature: { label: "Nature", icon: Trees },
  animals: { label: "Animals", icon: PawPrint },
  verbs: { label: "Common Verbs", icon: Zap },
  adjectives: { label: "Adjectives", icon: SlidersHorizontal },
};

export const CATEGORIES: Record<Category, [string, string][]> = {
  greetings: [
    ["merhaba", "hello"],
    ["selam", "hi"],
    ["günaydın", "good morning"],
    ["iyi geceler", "good night"],
    ["iyi akşamlar", "good evening"],
    ["hoşça kal", "goodbye"],
    ["güle güle", "goodbye"],
    ["teşekkürler", "thank you"],
    ["teşekkür ederim", "thank you"],
    ["rica ederim", "you're welcome"],
    ["lütfen", "please"],
    ["evet", "yes"],
    ["hayır", "no"],
    ["tamam", "okay"],
    ["affedersiniz", "excuse me"],
    ["özür dilerim", "sorry"],
    ["nasılsın", "how are you"],
    ["iyiyim", "I'm fine"],
    ["hoşgeldin", "welcome"],
    ["hoşbulduk", "glad to be here"],
  ],
  daily_expressions: [
    ["günaydın", "good morning"],
    ["iyi günler", "good day / good afternoon"],
    ["iyi akşamlar", "good evening"],
    ["iyi geceler", "good night"],
    ["hoşça kal", "take care / goodbye"],
    ["güle güle", "goodbye / see you"],
    ["hoş geldin", "welcome"],
    ["hoş geldiniz", "welcome (formal)"],
    ["hoş bulduk", "nice to meet you / welcome"],
    ["teşekkür ederim", "thank you"],
    ["sağ ol", "thanks / thank you"],
    ["rica ederim", "you're welcome"],
    ["bir şey değil", "no problem / it's nothing"],
    ["özür dilerim", "sorry / excuse me"],
    ["önemli değil", "it's okay / no worries"],
    ["sorun değil", "no problem / it's okay"],
    ["lütfen", "please"],
    ["tamam", "okay / alright"],
    ["afiyet olsun", "enjoy your meal / bon appétit"],
    ["elinize sağlık", "to your health / bless you"],
    ["çok yaşa", "long live / cheers"],
    ["geçmiş olsun", "get well soon"],
    ["iyi dersler", "have a good lesson / good luck in class"],
    ["kolay gelsin", "good luck / may it be easy"],
    ["bol şanslar", "good luck"],
    ["iyi yolculuklar", "have a nice trip"],
    ["tebrikler", "congratulations"],
    ["kutlarım", "i congratulate you"],
    ["çok teşekkür ederim", "thank you very much"],
    ["doğum günün kutlu olsun", "happy birthday"],
    ["nice senelere", "many happy returns"],
    ["mutlu yıllar", "happy years / best wishes"],
    ["iyi seneler", "happy new years / best wishes"],
    ["anneler günün kutlu olsun", "happy mother's day"],
    ["iyi bayramlar", "happy holidays"],
    ["bayramınız kutlu olsun", "may your holiday be blessed"],
    ["sonsuz mutluluklar", "endless happiness"],
    ["merhaba", "hello"],
    ["selam", "hi"],
    ["nasılsın", "how are you"],
    ["iyiyim", "i'm fine"],
    ["ne haber", "what's up"],
    ["görüşürüz", "see you"],
    ["sonra görüşürüz", "see you later"],
    ["ben Türkçe öğreniyorum", "i am learning Turkish"],
    ["anlamadım", "i didn't understand"],
    ["tekrar eder misiniz", "could you say it again"],
    ["lütfen yavaş konuşun", "please speak slowly"],
  ],
  pronouns: [
    ["ben", "I"],
    ["sen", "you"],
    ["o", "he/she/it"],
    ["biz", "we"],
    ["siz", "you (formal)"],
    ["onlar", "they"],
  ],
  questions: [
    ["ne", "what"],
    ["kim", "who"],
    ["nerede", "where"],
    ["ne zaman", "when"],
    ["neden", "why"],
    ["nasıl", "how"],
    ["hangi", "which"],
    ["kaç", "how many"],
    ["kaçıncı", "which one / what order"],
    ["niçin", "why"],
  ],
  numbers: [
    ["bir", "one"],
    ["iki", "two"],
    ["üç", "three"],
    ["dört", "four"],
    ["beş", "five"],
    ["altı", "six"],
    ["yedi", "seven"],
    ["sekiz", "eight"],
    ["dokuz", "nine"],
    ["on", "ten"],
    ["yirmi", "twenty"],
    ["otuz", "thirty"],
    ["yüz", "hundred"],
    ["bin", "thousand"],
    ["sayı", "number"],
    ["artı", "plus"],
    ["eksi", "minus"],
    ["çarpı", "multiplied by"],
    ["bölü", "divided by"],
  ],
  days: [
    ["pazartesi", "Monday"],
    ["salı", "Tuesday"],
    ["çarşamba", "Wednesday"],
    ["perşembe", "Thursday"],
    ["cuma", "Friday"],
    ["cumartesi", "Saturday"],
    ["pazar", "Sunday"],
  ],
  time: [
    ["gün", "day"],
    ["hafta", "week"],
    ["ay", "month"],
    ["yıl", "year"],
    ["saat", "hour"],
    ["dakika", "minute"],
    ["saniye", "second"],
    ["sabah", "morning"],
    ["öğle", "noon"],
    ["öğleden sonra", "afternoon"],
    ["gündüz", "daytime"],
    ["akşam", "evening"],
    ["gece", "night"],
    ["tarih", "date"],
    ["takvim", "calendar"],
    ["bugün", "today"],
    ["yarın", "tomorrow"],
    ["dün", "yesterday"],
    ["şimdi", "now"],
    ["erken", "early"],
    ["geç", "late"],
    ["önce", "before"],
    ["sonra", "later"],
    ["yakında", "soon"],
    ["hemen", "immediately"],
    ["bazen", "sometimes"],
    ["her zaman", "always"],
    ["hafta sonu", "weekend"],
  ],
  family: [
    ["anne", "mother"],
    ["baba", "father"],
    ["kardeş", "sibling"],
    ["abla", "older sister"],
    ["ağabey", "older brother"],
    ["kız", "girl"],
    ["oğul", "son"],
    ["çocuk", "child"],
    ["aile", "family"],
    ["arkadaş", "friend"],
    ["eş", "spouse"],
  ],
  body: [
    ["kafa", "head"],
    ["göz", "eye"],
    ["kulak", "ear"],
    ["burun", "nose"],
    ["ağız", "mouth"],
    ["diş", "tooth"],
    ["saç", "hair"],
    ["boyun", "neck"],
    ["omuz", "shoulder"],
    ["kol", "arm"],
    ["el", "hand"],
    ["parmak", "finger"],
    ["bacak", "leg"],
    ["diz", "knee"],
    ["ayak", "foot"],
    ["kalp", "heart"],
    ["akciğer", "lung"],
    ["karın", "stomach"],
  ],
  colors: [
    ["kırmızı", "red"],
    ["mavi", "blue"],
    ["yeşil", "green"],
    ["sarı", "yellow"],
    ["siyah", "black"],
    ["beyaz", "white"],
    ["turuncu", "orange"],
    ["mor", "purple"],
    ["pembe", "pink"],
    ["kahverengi", "brown"],
    ["gri", "gray"],
  ],
  food: [
    ["su", "water"],
    ["ekmek", "bread"],
    ["süt", "milk"],
    ["çay", "tea"],
    ["kahve", "coffee"],
    ["et", "meat"],
    ["tavuk", "chicken"],
    ["peynir", "cheese"],
    ["yumurta", "egg"],
    ["elma", "apple"],
    ["muz", "banana"],
    ["portakal", "orange (fruit)"],
    ["şeker", "sugar"],
    ["tuz", "salt"],
    ["yemek", "food"],
    ["kahvaltı", "breakfast"],
    ["akşam yemeği", "dinner"],
  ],
  vegetables: [
    ["domates", "tomato"],
    ["patates", "potato"],
    ["soğan", "onion"],
    ["sarımsak", "garlic"],
    ["havuç", "carrot"],
    ["salatalık", "cucumber"],
    ["biber", "pepper"],
    ["patlıcan", "eggplant"],
    ["kabak", "zucchini"],
    ["lahana", "cabbage"],
    ["marul", "lettuce"],
    ["ıspanak", "spinach"],
    ["bezelye", "peas"],
    ["fasulye", "beans"],
    ["brokoli", "broccoli"],
    ["karnabahar", "cauliflower"],
    ["mısır", "corn"],
  ],
  household: [
    ["ev", "house"],
    ["oda", "room"],
    ["oturma odası", "living room"],
    ["yatak odası", "bedroom"],
    ["yemek odası", "dining room"],
    ["mutfak", "kitchen"],
    ["banyo", "bathroom"],
    ["tuvalet", "toilet"],
    ["çalışma odası", "office"],
    ["misafir odası", "guest room"],
    ["çocuk odası", "children's room"],
    ["koridor", "hallway"],
    ["çatı katı", "attic"],
    ["bodrum kat", "basement"],
    ["garaj", "garage"],
    ["bahçe", "garden"],
    ["balkon", "balcony"],
    ["kapı", "door"],
    ["pencere", "window"],
    ["çatı", "roof"],
    ["duvar", "wall"],
    ["zemin", "floor"],
    ["lamba", "lamp"],
    ["koltuk", "armchair"],
    ["masa", "table"],
    ["sandalye", "chair"],
    ["televizyon", "television"],
    ["halı", "carpet"],
    ["perde", "curtain"],
    ["yatak", "bed"],
    ["dolap", "cabinet"],
    ["komodin", "nightstand"],
    ["yastık", "pillow"],
    ["nevresim", "bedding"],
    ["kitaplık", "bookshelf"],
    ["çalışma masası", "desk"],
    ["çalışma sandalyesi", "desk chair"],
    ["oyuncak", "toy"],
    ["mutfak dolabı", "kitchen cabinet"],
    ["buzdolabı", "refrigerator"],
    ["ocak", "stove"],
    ["fırın", "oven"],
    ["lavabo", "sink"],
    ["tabak", "plate"],
    ["bardak", "glass"],
    ["çatal", "fork"],
    ["kaşık", "spoon"],
    ["tencere", "pot"],
    ["duş", "shower"],
    ["tuvalet", "toilet"],
    ["lavabonun aynası", "bathroom mirror"],
    ["havlu", "towel"],
    ["çamaşır makinesi", "washing machine"],
    ["merdiven", "stairs"],
    ["ayakkabılık", "shoe rack"],
    ["balkon masası", "balcony table"],
    ["saksı", "flowerpot"],
    ["garaj kapısı", "garage door"],
    ["araba", "car"],
    ["raf", "shelf"],
    ["kutu", "box"],
    ["bahçe kapısı", "garden gate"],
    ["ağaç", "tree"],
    ["çiçek", "flower"],
    ["salıncak", "swing"],
    ["anahtar", "key"],
    ["çöp kutusu", "dustbin"],
    ["sehpa", "coffee table"],
    ["yazı tahtası", "whiteboard"],
  ],
  places: [
    ["adres", "address"],
    ["alışveriş", "shopping"],
    ["alışveriş merkezi", "shopping mall"],
    ["anahtar", "key"],
    ["banka", "bank"],
    ["bina", "building"],
    ["durak", "station / stop"],
    ["eczacı", "pharmacist"],
    ["gazete", "newspaper"],
    ["hastane", "hospital"],
    ["istasyon", "station"],
    ["kantin", "canteen"],
    ["kasap", "butcher"],
    ["kasiyer", "cashier"],
    ["kuaför", "hairdresser"],
    ["kuyumcu", "jeweler"],
    ["mahalle", "neighborhood"],
    ["manav", "greengrocer"],
    ["market", "market"],
    ["ofis", "office"],
    ["pastane", "bakery / pastry shop"],
    ["postane", "post office"],
    ["projektör", "projector"],
    ["resepsiyonist", "receptionist"],
    ["semt", "district"],
    ["şehir", "city"],
    ["yer", "place / floor"],
  ],
  nature: [
    ["güneş", "sun"],
    ["yıldız", "star"],
    ["gökyüzü", "sky"],
    ["deniz", "sea"],
    ["dağ", "mountain"],
    ["ağaç", "tree"],
    ["çiçek", "flower"],
    ["lale", "tulip"],
    ["yol", "road"],
    ["şehir", "city"],
    ["ülke", "country"],
  ],
  animals: [
    ["köpek", "dog"],
    ["kedi", "cat"],
    ["kuş", "bird"],
    ["balık", "fish"],
  ],
  verbs: [
    ["gitmek", "to go"],
    ["gelmek", "to come"],
    ["içmek", "to drink"],
    ["görmek", "to see"],
    ["bilmek", "to know"],
    ["istemek", "to want"],
    ["sevmek", "to love"],
    ["yapmak", "to do"],
    ["almak", "to take"],
    ["vermek", "to give"],
    ["konuşmak", "to speak"],
    ["anlamak", "to understand"],
    ["okumak", "to read"],
    ["yazmak", "to write"],
    ["duymak", "to hear"],
    ["uyumak", "to sleep"],
    ["çalışmak", "to work"],
    ["oynamak", "to play"],
    ["düşünmek", "to think"],
    ["beklemek", "to wait"],
    ["başlamak", "to start"],
    ["bitirmek", "to finish"],
    ["olmak", "to be"],
    ["demek", "to say"],
    ["sormak", "to ask"],
    ["gülmek", "to laugh"],
    ["kullanmak", "to use"],
    ["bulmak", "to find"],
    ["oturmak", "to sit"],
    ["kalkmak", "to stand up"],
    ["açmak", "to open"],
    ["kapatmak", "to close"],
    ["yemek", "to eat"],
    ["uyandırmak", "to wake up"],
    ["öğrenmek", "to learn"],
    ["hatırlamak", "to remember"],
    ["sürmek", "to drive / continue"],
    ["araştırmak", "to research / look into"],
    ["kayıt olmak", "to register / sign up"],
    ["aramak", "to call / to search"],
  ],
  adjectives: [
    ["büyük", "big"],
    ["küçük", "small"],
    ["iyi", "good"],
    ["kötü", "bad"],
    ["güzel", "beautiful"],
    ["çirkin", "ugly"],
    ["yeni", "new"],
    ["eski", "old"],
    ["sıcak", "hot"],
    ["soğuk", "cold"],
    ["hızlı", "fast"],
    ["yavaş", "slow"],
    ["mutlu", "happy"],
    ["üzgün", "sad"],
    ["yorgun", "tired"],
    ["zengin", "rich"],
    ["fakir", "poor"],
    ["uzun", "long"],
    ["kısa", "short"],
    ["kolay", "easy"],
    ["zor", "difficult"],
    ["açık", "open / light"],
    ["başarılı", "successful"],
    ["bekâr", "single"],
    ["boş", "empty"],
    ["cesur", "brave"],
    ["ciddi", "serious"],
    ["çalışkan", "hardworking"],
    ["dağınık", "messy"],
    ["dar", "narrow"],
    ["dikkatli", "careful"],
    ["dikkatsiz", "careless"],
    ["dolu", "full"],
    ["düzenli", "regular / tidy"],
    ["eğlenceli", "entertaining / fun"],
    ["evli", "married"],
    ["genç", "young"],
    ["geniş", "wide"],
    ["güçlü", "strong"],
    ["güçsüz", "weak"],
    ["aç", "hungry"],
    ["hasta", "sick"],
    ["kibar", "polite"],
    ["komik", "funny"],
    ["konuşkan", "talkative"],
    ["mutsuz", "unhappy"],
    ["önemli", "important"],
    ["önemsiz", "unimportant"],
    ["pahalı", "expensive"],
    ["sakin", "calm"],
    ["sessiz", "silent / quiet"],
    ["sosyal", "social"],
    ["sıfat", "adjective"],
    ["sinirli", "angry"],
    ["tehlikeli", "dangerous"],
    ["tembel", "lazy"],
    ["tuzlu", "salty"],
    ["ucuz", "cheap"],
    ["uzak", "far"],
    ["yakın", "near"],
    ["yaşlı", "old"],
    ["yoğun", "busy / intense"],
    ["zayıf", "weak / thin"],
  ],
};

export const TR_EN: Dictionary = {};
export const EN_TR: Dictionary = {};
CATEGORY_ORDER.forEach((cat) => {
  CATEGORIES[cat].forEach(([t, e]) => {
    TR_EN[t] = e;
    const key = e.toLowerCase();
    if (!EN_TR[key]) EN_TR[key] = t;
  });
});

// A lightweight stand-in for "pictures": emoji for concrete, visual words.
// Abstract words (pronouns, question words, most verbs/adjectives) are left
// out on purpose — a picture wouldn't add much there.
export const EMOJI: Dictionary = {
  merhaba: "👋",
  selam: "👋",
  günaydın: "🌅",
  "iyi geceler": "🌙",
  "iyi akşamlar": "🌆",
  "hoşça kal": "👋",
  "güle güle": "👋",
  teşekkürler: "🙏",
  "teşekkür ederim": "🙏",
  evet: "✅",
  hayır: "❌",
  tamam: "👍",
  hoşgeldin: "🎉",
  hoşbulduk: "🎉",
  bir: "1️⃣",
  iki: "2️⃣",
  üç: "3️⃣",
  dört: "4️⃣",
  beş: "5️⃣",
  altı: "6️⃣",
  yedi: "7️⃣",
  sekiz: "8️⃣",
  dokuz: "9️⃣",
  on: "🔟",
  yüz: "💯",
  pazartesi: "📅",
  salı: "📅",
  çarşamba: "📅",
  perşembe: "📅",
  cuma: "📅",
  cumartesi: "📅",
  pazar: "📅",
  gün: "☀️",
  hafta: "🗓️",
  ay: "🌙",
  yıl: "📆",
  saat: "⏰",
  dakika: "⏱️",
  bugün: "📍",
  yarın: "➡️",
  dün: "⬅️",
  anne: "👩",
  baba: "👨",
  kardeş: "👫",
  abla: "👧",
  ağabey: "👦",
  kız: "👧",
  oğul: "👦",
  çocuk: "🧒",
  aile: "👪",
  arkadaş: "🧑‍🤝‍🧑",
  eş: "💑",
  kafa: "🧠",
  göz: "👀",
  kulak: "👂",
  burun: "👃",
  ağız: "👄",
  diş: "🦷",
  saç: "💇",
  boyun: "🫦",
  omuz: "🫱",
  kol: "💪",
  el: "🖐️",
  parmak: "🫶",
  bacak: "🦵",
  diz: "🦿",
  ayak: "🦶",
  kalp: "💖",
  akciğer: "🫁",
  karın: "🩺",
  kırmızı: "🔴",
  mavi: "🔵",
  yeşil: "🟢",
  sarı: "🟡",
  siyah: "⚫",
  beyaz: "⚪",
  turuncu: "🟠",
  mor: "🟣",
  pembe: "🌸",
  kahverengi: "🟤",
  gri: "⬜",
  su: "💧",
  ekmek: "🍞",
  süt: "🥛",
  çay: "🍵",
  kahve: "☕",
  et: "🥩",
  tavuk: "🍗",
  peynir: "🧀",
  yumurta: "🥚",
  elma: "🍎",
  muz: "🍌",
  portakal: "🍊",
  şeker: "🍬",
  tuz: "🧂",
  yemek: "🍽️",
  kahvaltı: "🍳",
  "akşam yemeği": "🍽️",
  domates: "🍅",
  patates: "🥔",
  soğan: "🧅",
  sarımsak: "🧄",
  havuç: "🥕",
  salatalık: "🥒",
  biber: "🌶️",
  patlıcan: "🍆",
  kabak: "🎃",
  lahana: "🥬",
  marul: "🥬",
  ıspanak: "🥬",
  bezelye: "🫛",
  fasulye: "🫘",
  brokoli: "🥦",
  karnabahar: "🥦",
  mısır: "🌽",
  ev: "🏠",
  oda: "�️",
  mutfak: "🍳",
  banyo: "🛁",
  kapı: "🚪",
  pencere: "🪟",
  çatı: "🏠",
  duvar: "🧱",
  zemin: "🧱",
  lamba: "💡",
  koltuk: "🛋️",
  masa: "🪵",
  sandalye: "🪑",
  televizyon: "📺",
  halı: "🧶",
  perde: "🪟",
  yatak: "🛏️",
  dolap: "🗄️",
  komodin: "🗄️",
  yastık: "🛏️",
  nevresim: "🛏️",
  kitaplık: "📚",
  "çalışma masası": "🖥️",
  "çalışma sandalyesi": "🪑",
  oyuncak: "🧸",
  "mutfak dolabı": "🗄️",
  buzdolabı: "🧊",
  ocak: "🔥",
  fırın: "🔥",
  lavabo: "🚰",
  tabak: "🍽️",
  bardak: "🥤",
  çatal: "🍴",
  kaşık: "🥄",
  tencere: "🍲",
  duş: "🚿",
  tuvalet: "🚽",
  "lavabonun aynası": "🪞",
  havlu: "🧼",
  "çamaşır makinesi": "🧺",
  merdiven: "🪜",
  ayakkabılık: "👟",
  "balkon masası": "☀️",
  saksı: "🪴",
  "garaj kapısı": "🚗",
  araba: "🚗",
  raf: "🧰",
  kutu: "📦",
  "bahçe kapısı": "🚪",
  ağaç: "🌳",
  çiçek: "🌸",
  salıncak: "🛝",
  ayna: "🪞",
  adres: "📍",
  alışveriş: "🛍️",
  "alışveriş merkezi": "🏬",
  anahtar: "🔑",
  banka: "🏦",
  bina: "🏢",
  durak: "🛑",
  eczacı: "💊",
  gazete: "📰",
  hastane: "🏥",
  istasyon: "🚉",
  kantin: "🍽️",
  kasap: "🥩",
  kasiyer: "💰",
  kuaför: "💇",
  kuyumcu: "💍",
  mahalle: "🏘️",
  manav: "🥬",
  market: "🛒",
  ofis: "💼",
  pastane: "🥐",
  postane: "📮",
  projektör: "📽️",
  resepsiyonist: "🧑‍💼",
  semt: "📍",
  şehir: "🏙️",
  yer: "📍",
  açık: "🔓",
  başarılı: "✅",
  bekâr: "💍",
  boş: "🫙",
  cesur: "🦁",
  ciddi: "🧠",
  çalışkan: "💪",
  dağınık: "🧹",
  dar: "🚧",
  dikkatli: "⚠️",
  dikkatsiz: "⚠️",
  dolu: "📦",
  düzenli: "🧹",
  eğlenceli: "🎉",
  evli: "💑",
  genç: "🧑",
  geniş: "🌍",
  güçlü: "💪",
  güçsüz: "😵",
  kibar: "🤝",
  komik: "😄",
  konuşkan: "🗣️",
  mutsuz: "😞",
  önemli: "❗",
  önemsiz: "➖",
  pahalı: "💸",
  sakin: "🌊",
  sessiz: "🤫",
  sosyal: "👥",
  sinirli: "😠",
  tehlikeli: "⚠️",
  tembel: "😴",
  tuzlu: "🧂",
  ucuz: "💵",
  uzak: "🗺️",
  yakın: "📍",
  yaşlı: "👵",
  yoğun: "📈",
  zayıf: "🩺",
  artı: "➕",
  eksi: "➖",
  çarpı: "✖️",
  bölü: "➗",
  kaçıncı: "❓",
  niçin: "❓",
  aramak: "📞",
  güneş: "☀️",
  yıldız: "⭐",
  gökyüzü: "☁️",
  deniz: "🌊",
  dağ: "⛰️",
  lale: "🌷",
  yol: "🛣️",
  şehir: "🏙️",
  ülke: "🌍",
  köpek: "🐶",
  kedi: "🐱",
  kuş: "🐦",
  balık: "🐟",
  büyük: "🐘",
  küçük: "🐜",
  iyi: "👍",
  kötü: "👎",
  güzel: "😍",
  çirkin: "😬",
  yeni: "🆕",
  eski: "🕰️",
  sıcak: "🔥",
  soğuk: "❄️",
  hızlı: "⚡",
  yavaş: "🐌",
  mutlu: "😊",
  üzgün: "😢",
  yorgun: "😴",
  zengin: "💰",
  kısa: "✂️",
};
